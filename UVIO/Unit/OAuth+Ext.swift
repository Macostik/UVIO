...
extension OAuth2Swift {
    ...
    struct AuthorizePublisher: Publisher {
        ...
        fileprivate init(oauth2: OAuth2Swift,
                         callbackURL: URLConvertible?,
                         scope: String,
                         state: String = "",
    ...
    where S.Input == TokenSuccess, S.Failure == OAuthSwiftError {
        ...
        init(oauth2: OAuth2Swift,
             subscriber: S) {
            self.oauth2 = oauth2
            self.subscriber = subscriber
        }
        func authorize(callbackURL: URLConvertible?,
                       scope: String,
                       state: String,
                       parameters: Parameters = [:],
                       headers: OAuthSwift.Headers? = nil) {
            handle?.cancel()
            handle = oauth2.authorize(withCallbackURL: callbackURL,
                                      scope: scope,
                                      state: state,
